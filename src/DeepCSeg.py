...
import cv2
import argparse
sys.path.append("/home/ext_gnanaolivu_rohandavid_mayo_e/Mask_RCNN/")
import sys
...
parser = argparse.ArgumentParser(description="Prepare inputs for cell segmentation")
parser.add_argument('--dapi-dir', required=True, help='Path to DAPI image directory')
parser.add_argument('--membrane-dir', required=True, help='Path to membrane image directory')
parser.add_argument('--biomarker-dir', required=True, help='Path to biomarker image directory')
parser.add_argument('--dapi-id', required=True, help='Filename (without extension) of DAPI image')
parser.add_argument('--membrane-id', required=True, help='Filename (without extension) of membrane image')
parser.add_argument('--biomarker-id', required=True, help='Filename (without extension) of biomarker image')
parser.add_argument('--model-path', default='/home/ext_gnanaolivu_rohandavid_mayo_e/deepcseg/models', help='Path to model directory')
parser.add_argument('--result-path', default='/home/ext_gnanaolivu_rohandavid_mayo_e/deepcseg/results', help='Path to result output directory')
parser.add_argument('--segmentation', default='YES', choices=['YES', 'NO'], help='Whether to perform segmentation')
parser.add_argument('--nuclei-result', default='nuclei_result.dat', help='Output file for nuclei segmentation')
parser.add_argument('--cell-result', default='cell_result.dat', help='Output file for cell segmentation')
...
#DATASET_DIR = "/home/ext_gnanaolivu_rohandavid_mayo_e/deepcseg/dapi_img"      # dapi image directory
DATASET_DIR = args.dapi_dir
#DATASET_DIR_MEM = "/home/ext_gnanaolivu_rohandavid_mayo_e/deepcseg/membrane_img"   # membrane image directory
DATASET_DIR_MEM = args.membrane_dir
#DATASET_DIR_BIOMARKER = "/home/ext_gnanaolivu_rohandavid_mayo_e/deepcseg/biomarker_img"    # bio-marker image directory
DATASET_DIR_BIOMARKER = args.biomarker_dir
SEGMENTATION =  args.segmentation #"YES"  # if performing segmentation or not, if not using the existing segmentation resutls

VAL_DAPI_IDS = [args.dapi_id] #["ROI_01_DAPI"]          # dapi image name
VAL_MEM_ID =[args.membrane_id] # ["ROI_01_NaKATPase"]              # membrane image name
VAL_BIOMARKER_ID = [args.biomarker_id] #["ROI_01_PCK"]  # test biomarker image name
    
RESULT_NUCLEI = args.nuclei_result #"ROI_01_nuclei_result.dat"   # result filename for nucleus segmentation
RESULT_CELL = args.cell_result #"ROI_01_cell_result.dat"      # result filename for cell segmentation
MODEL_PATH = args.model_path #"/home/ext_gnanaolivu_rohandavid_mayo_e/deepcseg/models"  # directory where the trained models saved
RESULT_PATH = args.result_path # "/home/ext_gnanaolivu_rohandavid_mayo_e/deepcseg/results"   # results directory
...
MEM_filename = FindImgFileName(DATASET_DIR_MEM, VAL_MEM_ID)   # get the mem image filename
...
class Configuration(Config):
   ...
   # IMAGE_MAX_DIM = 512
    ...
    DETECTION_MAX_INSTANCES = math.ceil((HightOfImg/256)) * math.ceil((WidthOfImg/256)) * 600
...
def update_prediction_result(results, meta, padding):
    ...
    masksIdx = []
    masksIdxOri = []
    mask_ori_size = (0, 0)   #added by Rohan
    ...
    for j in range(0, numofmasks):  # format the indices to match the original input image
        ...
        w = round(mask_int.shape[0]*scale)
        h = round(mask_int.shape[1]*scale)
...
if os.path.isfile(infer_writefilename) and os.path.isfile(infer2_writefilename) and SEGMENTATION == 'NO':
    ...
    print(json.dumps(filenames)) # return the filenames in json format as response; plug-in uses filenames to retrieve the results.
